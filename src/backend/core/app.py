...
from models.database import db, Device, Reading, Alert, User, Settings, Notification, OTPCode, PasswordResetToken, UserSession

# Import new services
from services.notification_service import MobileNotificationService
from services.auth_service import EnhancedAuthenticationService
from services.forecasting_service import forecaster

# Import production utilities
from utils.logger import setup_logger, log_api_call, log_audit, log_error_with_context
from utils.validation import validate_json, validate_sensor_data, validate_username, validate_password, validate_mobile_number
from middleware.rate_limiter import rate_limit, rate_limit_moderate, rate_limit_strict, rate_limiter

# Import scripts (they will run from their location)
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / 'scripts'))
from maintenance.backup_manager import DatabaseBackup, auto_backup_scheduler
from maintenance.scheduled_tasks import ScheduledTasks, run_manual_cleanup

app = Flask(__name__)

# Configuration  
# app.py is in src/backend/core/, PROJECT_ROOT is 3 levels up
...
def initialize_database():
    ...
    with app.app_context():
        ...
        if User.query.count() == 0:
            ...
            admin.set_password('admin123')  # Default password: admin123
            db.session.add(admin)
            print("✓ Created default admin user")
            print("  Username: admin")
            print("  Password: admin123")
            print("  Mobile: +11234567890")
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        logger.debug(f"login_required check - user_id in session: {'user_id' in session}")
...
def admin_required(f):
    ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
        logger.debug(f"admin_required check - Session keys: {list(session.keys())}")
        logger.debug(f"admin_required check - user_id in session: {'user_id' in session}")
        ...
        if not user:
            logger.warning(f"User ID {session['user_id']} in session but not found in database")
            session.clear()
            return jsonify({'error': 'Session invalid. Please login again.'}), 401
            
        if user.role != 'admin':
            logger.warning(f"Non-admin user {user.username} attempted to access admin endpoint: {request.path}")
            return jsonify({'error': 'Admin access required. You must be logged in as an administrator.'}), 403
...
@login_required
@rate_limit_moderate
@validate_json(required_fields=['mobile_number'])
def send_mobile_otp():
    ...
    mobile_alerts_setting = Settings.query.filter_by(key='enable_mobile_alerts').first()
    if not mobile_alerts_setting or mobile_alerts_setting.value != 'true':
        return jsonify({
            'success': False,
            'error': 'Mobile notifications are not enabled. Please contact administrator.'
        }), 400
    
    # Check if mobile notification service is available
    if not mobile_notification.enabled:
        return jsonify({
            'success': False,
            'error': 'Mobile notification service is not configured. Please contact administrator.'
        }), 400
    ...
    # Send OTP
    result = auth_service.send_verification_otp(user.id, mobile_number)
    
    if result['success']:
        log_audit(logger, 'mobile_otp_sent', user.username, f'Mobile: {mobile_number}')
    
    return jsonify(result), 200 if result['success'] else 400


@app.route('/api/auth/update-alert-preferences', methods=['POST'])
@login_required
@rate_limit_moderate
...
def auth_status():
    ...
    if 'user_id' in session:
        ...
        if user:
            ...
            # User ID in session but user not found in DB
            logger.warning(f"Session has user_id {session['user_id']} but user not found in database")
            session.clear()
...
def reinitialize_mobile_service():
    """Reinitialize mobile notification service with current settings"""
    ...
    try:
        ...
        # Update auth service with new notification service
        auth_service.notification_service = mobile_notification
        
        logger.info("Mobile notification service reinitialized")
        return True
    except Exception as e:
        logger.error(f"Failed to reinitialize mobile service: {e}")
        return False
...
@app.route('/api/admin/test-twilio', methods=['POST'])
@login_required
@admin_required
def test_twilio_connection():
    """Test Twilio API connection by sending test SMS"""
    try:
        # Reinitialize service to get latest settings
        reinitialize_mobile_service()
        
        if not mobile_notification.enabled:
            return jsonify({
                'success': False,
                'error': 'Mobile notifications not enabled. Please configure Twilio credentials and enable in settings.'
            }), 400
        
        # Get admin mobile number from settings
        admin_mobile_setting = Settings.query.filter_by(key='admin_mobile_number').first()
        admin_mobile = admin_mobile_setting.value if admin_mobile_setting else None
        
        if not admin_mobile:
            return jsonify({
                'success': False,
                'error': 'Admin mobile number not configured. Please add it in Twilio Configuration section.'
            }), 400
        
        # Test connection with actual SMS
        result = mobile_notification.test_connection(test_phone_number=admin_mobile)
        
        if result['success']:
            log_audit(logger, 'twilio_test_success', session.get('username'), f"Test SMS sent to {admin_mobile}")
            return jsonify(result)
        else:
            log_audit(logger, 'twilio_test_failed', session.get('username'), f"Error: {result.get('error')}")
            return jsonify(result), 400
            
    except Exception as e:
        log_error_with_context(logger, e, {'action': 'test_twilio'})
        return jsonify({
            'success': False,
